...
\nabla x_t=x_t-x_{t-1} \\
\phi(B)=1-\phi_1 B-\phi_2 B^2-\phi_3 B^3-\phi_4 B^4 \\
\phi(B)=1+0.1997 B+0.9276 B^2 -0.0075 B^3-0.0154 B^4\\
\theta(B)=1+\theta_1 B+\theta_2 B^2 \\
\theta(B)=1+0.2130 B+0.9248 B^2 \\
(1+0.1997 B+0.9276 B^2 -0.0075 B^3-0.0154 B^4)(1-B) x_t= (1+0.2130 B+0.9248 B^2)w_t
\end{gathered}
$$
...
$$
\begin{gathered}
(1-0.8003 B+0.7279 B^2 -0.9351 B^3 -0.0079 B^4+0.0154 B^5) x_t = (1+0.2130 B+0.9248 B^2)w_t \\
x_t-0.8003 B x_t+0.7279 B^2 x_t -0.9351 B^3 x_t-0.0079 B^4 x_t +0.0154 B^5 x_t = w_t+0.2130 B w_t+0.9248 B^2 w_t \\
x_t = 0.8003 B x_t - 0.7279 B^2 x_t + 0.9351 B^3 x_t + 0.0079 B^4 x_t - 0.0154 B^5 x_t + w_t+0.2130 B w_t+0.9248 B^2 w_t \\
w_t=\sigma_t \epsilon_t \\
var\left(w_t \mid w_{t-1}\right)=\sigma_t^2=\alpha_0+\alpha_1 w_{t-1}^2+\beta_1 \sigma_{t-1}^2 \\
\sigma_t^2=0.000000095+0.036 w_{t-1}^2+0.96 \sigma_{t-1}^2
\end{gathered}
$$
...
We were comparing Method 1: ARIMA(4,1,2) + GARCH(1,1) and Method 2: GARCH(1,1). <!-- In fact, these two models are essentially the same, because differencing the log-transformed prices in ARIMA(0,1,0) gives us the log returns, which are the direct input for the GARCH(1,1) model in Method 2. -->
